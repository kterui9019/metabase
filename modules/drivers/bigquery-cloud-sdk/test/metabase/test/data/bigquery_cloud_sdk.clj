(ns metabase.test.data.bigquery-cloud-sdk
  ...
  (:import [com.google.cloud.bigquery BigQuery BigQuery$DatasetDeleteOption BigQuery$DatasetListOption
                                      BigQuery$DatasetOption BigQuery$TableOption DatasetInfo Field LegacySQLTypeName
                                      InsertAllRequest InsertAllResponse Schema StandardTableDefinition TableId
...
(defn- create-dataset! [^String dataset-id]
  {:pre [(seq dataset-id)]}
  (.create (bigquery) (DatasetInfo/of dataset-id) (u/varargs BigQuery$DatasetOption))
  (println (u/format-color 'blue "Created BigQuery dataset `%s.%s`." (project-id) dataset-id)))

(defn- destroy-dataset! [^String dataset-id]
  {:pre [(seq dataset-id)]}
  (.delete (bigquery) dataset-id (u/varargs BigQuery$DatasetDeleteOption))
  ...
  (u/ignore-exceptions
   ...
   (let [tbl-id (TableId/of dataset-id table-id)
         schema (Schema/of (u/varargs Field (for [[field-name field-type] field-name->type]
                                              (Field/of
                                                field-name
                                                (LegacySQLTypeName/valueOf field-type)
                                                (into-array Field [])))))
...
(defn- rows->request ^InsertAllRequest [^String dataset-id ^String table-id row-maps]
  (let [insert-data (reduce (fn [acc row-map]
                              (assoc acc (get row-map :id) (->json row-map))) {} row-maps)]
    (InsertAllRequest/of (TableId/of dataset-id table-id) (HashMap. insert-data))))
